...
import {
    ...
    actionToLoginTrainerUserProfileByPhoneAndPasswordApiCall,
    actionToGetCurrentTrainerSessionDataApiCall, actionToGetAllScheduledLiveClassByTrainerId
...
commonRouter.post(
    ...
    expressAsyncHandler(async (req, res) => {
        ...
        const phone = req.body.phone;
        actionToLoginTrainerUserProfileByPhoneAndPasswordApiCall(phone,password)
            .then(user => {
                if(user?.id) {
                    createNewSessionWithUserDataAndRole(req, user).then(() => {
                        res.status(200).send({
