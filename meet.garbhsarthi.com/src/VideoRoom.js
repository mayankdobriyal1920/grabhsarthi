        ...
        async function init() {
            try {
                ...
                if (isTeacher) {
                    ...
                    socketRef.current.emit("request-join", {name: userName,roomId, userId});
                ...
                <div className={`main-video-container ${pinnedSocketId ? 'full-screen-container' : ''}`} ref={containerRef}>
                    ...
                    {!isTeacher && (
                        <div className={`main-video-grid participant ${pinnedSocketId && pinnedSocketId !== "local" ? "hidden" : ""}`}>
                            ...
                            <div className="actions-in-part-three">
                                <button className={`pin-badge ${pinnedSocketId === "local" ? "active" : ""}`} onClick={() => togglePin("local")}>
                                    {pinnedSocketId === "local" ? "Unpin" : "Pin"}
                                </button>
                            </div>
                        </div>
                    ...
                    {participants.map(
                        (p) =>
                            (p.role !== "teacher" && p.userId !== userId) && (
                                <ParticipantThumbnail
                                    key={p.socketId}
                                    p={p}
                                    isTeacher={isTeacher}
