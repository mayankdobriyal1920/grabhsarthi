...
const LiveClasses = () => {
    ...
    const copyMeetingUrl = async (meeting_link) => {
        if (!meeting_link) {
    ...
    const openMeetingUrl = async (meeting_link) => {
        if (!meeting_link) {
            toast("Meeting link not available", "warning");
            return;
        }
    ...
    const renderActionButtons = (cls) => {
        ...
        if (now.isSameOrAfter(start) && now.isBefore(end)) {
            return (
                <div className="join_and_copy_button">
                    <button
                        onClick={() => copyMeetingUrl(cls?.meeting_link)}
                    ...
                    <button
                        onClick={() => openMeetingUrl(cls?.meeting_link)}
                        disabled={!cls?.meeting_link}
                        className="subscription_page_button_live_class"
                    >
                        Open Link
