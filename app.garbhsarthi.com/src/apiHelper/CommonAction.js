...
export const actionToGenerateOtpForPhoneNumber = async (phoneNumber)=>{
    try {
        return await api.post(`actionToGenerateOtpForPhoneNumberApiCall`, {phone:phoneNumber,color:_generateRandomPastelColor()});
    } catch (error) {
        console.log(error);
    }
}
...
export const actionToSaveSelectedLiveClassDataData = (liveClassId) => {
    ...
    requestAllScheduledLiveClassData();
    try {
        api.post(`actionToSaveSelectedLiveClassDataDataApiCall`,{selected_live_class_id:liveClassId}).then(() => {
